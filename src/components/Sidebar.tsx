 ...
 * Sidebar.tsx
 *
 * Left panel on desktop, full-screen drawer on mobile.
 * Contains three tabs: Files, Mode, and Rubric.
 * On mobile, tapping outside or pressing the X closes it.
 */
...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar panel */}
      <div className="
        fixed inset-y-0 left-0 z-30 w-72 flex flex-col glass border-r border-slate-800 h-full
      ...
      ">
        ...
        <div className="p-4 border-b border-slate-800 flex-shrink-0">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-scholar-500 to-purple-600 flex items-center justify-center glow flex-shrink-0">
            ...
            <button
              onClick={() => setSidebarOpen(false)}
              className="sm:hidden w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-slate-400 hover:text-white transition-all"
            >
              ✕
            </button>
          </div>
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-gradient-to-r from-scholar-600 to-purple-600 text-white text-sm font-medium hover:from-scholar-500 hover:to-purple-500 transition-all flex items-center justify-center gap-2"
          >
            <span>+</span> New Chat
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3">
          ...
          {activePanel === 'files' && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                {documents.length > 0 && (
                  <button onClick={clearDocuments} className="text-xs text-red-400 hover:text-red-300 transition-colors">
                    Clear all
                  </button>
                )}
              </div>
              ...
              {documents.length === 0 ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-xs text-slate-700 mt-1">Upload files to enable smart search</p>
                </div>
              ) : (
                    ...
                    <div key={doc.id} className="glass rounded-lg p-2.5 group">
                      <div className="flex items-start gap-2">
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-red-400 transition-all text-xs flex-shrink-0"
                        >
                          ✕
                        </button>
                      </div>
                ...
                <div className="space-y-1.5">
                  {MODES.map(mode => (
                    ...
                    >
                      ...
                      {settings.mode === mode.value && <span className="ml-auto text-scholar-400 text-xs">✓</span>}
                    </button>
                  ))}
                </div>
              </div>
          ...
          {/* RUBRIC PANEL */}
          {activePanel === 'rubric' && <RubricPanel />}
        </div>
      </div>
    </>
  );
...
const RubricPanel: React.FC = () => {
  ...
  const updateCriteria = (id: string, updates: Partial<{ name: string; description: string; weight: number; enabled: boolean }>) => {
    if (!rubric) return;
    updateSettings({ rubric: { ...rubric, criteria: rubric.criteria.map(c => c.id === id ? { ...c, ...updates } : c) } });
  };

  const addCriteria = () => {
    if (!rubric) return;
    updateSettings({ rubric: { ...rubric, criteria: [...rubric.criteria, { id: Date.now().toString(), name: 'New Criterion', description: 'Describe what this evaluates', weight: 10, enabled: true }] } });
  };
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${settings.rubricEnabled ? 'bg-scholar-600' : 'bg-slate-700'}`}
        >
          <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${settings.rubricEnabled ? 'translate-x-4' : 'translate-x-1'}`} />
        </button>
      </div>
              ...
              <div key={criterion.id} className="glass rounded-lg p-3 space-y-2">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 transition-colors ${criterion.enabled ? 'bg-scholar-600 border-scholar-500' : 'border-slate-600'}`}
                  >
                    {criterion.enabled && <span className="text-white text-xs">✓</span>}
                  </button>
                  <input value={criterion.name} onChange={e => updateCriteria(criterion.id, { name: e.target.value })} className="flex-1 bg-transparent text-xs font-medium text-white focus:outline-none" />
                  <div className="flex items-center gap-1">
                    <input type="number" value={criterion.weight} onChange={e => updateCriteria(criterion.id, { weight: parseInt(e.target.value) || 0 })} className="w-10 bg-slate-800 border border-slate-700 rounded px-1 py-0.5 text-xs text-center text-white focus:outline-none" min="0" max="100" />
                    <span className="text-xs text-slate-500">%</span>
                  </div>
                  <button onClick={() => removeCriteria(criterion.id)} className="text-slate-600 hover:text-red-400 text-xs transition-colors">✕</button>
                </div>
                <textarea value={criterion.description} onChange={e => updateCriteria(criterion.id, { description: e.target.value })} className="w-full bg-slate-800/50 border border-slate-700 rounded px-2 py-1.5 text-xs text-slate-400 focus:outline-none focus:border-scholar-500 resize-none" rows={2} />
              </div>
            ))}
          </div>
          <button onClick={addCriteria} className="w-full py-2 rounded-xl border border-dashed border-slate-700 hover:border-scholar-500 text-xs text-slate-500 hover:text-scholar-400 transition-all">+ Add Criterion</button>
          <div className="text-xs text-slate-600 text-center">Total: {rubric.criteria.filter(c => c.enabled).reduce((s, c) => s + c.weight, 0)}%</div>
        </>
      )}
    </div>
  );
